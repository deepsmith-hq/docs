...
<Info>
  If you have more than one workspace, each one gets its own URL, `https://mcp.deepsmith.ai/mcp/ws-uuid/<workspace-id>`, which you'll find on **Settings → MCP server** in that workspace. Add one connector per workspace and name each one the way [Connecting multiple workspaces](#connecting-multiple-workspaces) describes.
</Info>
