...
DeepSmith fires webhooks for four event types across your content pipeline:

| Event | Description |
|-------|-------------|
| `content.status_updated` | An article's status changed (e.g., draft to published) |
| `opportunity_run.status_updated` | An opportunity agent run completed or failed |
| `agent_task.status_updated` | An AI agent task changed state |
| `iq.status_updated` | An IQ analysis completed or changed stage |

<Info>
  You can subscribe a single webhook to one or more events. Each webhook can listen to any combination of the available event types.
</Info>
